	...
	if(USART_GetITStatus(USART3, USART_IT_TC) != RESET)
	  {   
		    USART_ClearITPendingBit(USART3, USART_IT_TC);
				if(UART_Com3_Para.Send_Index < UART_Com3_Para.Send_Len)
					{
						UART_Com3_Para.Send_Index++;
						USART3->DR = (u16)UART_Com3_Para.buff [UART_Com3_Para.Send_Index];//					
					}
				else					
				UART_Com3_Para.SendStatus = True;
	  }
		
	if(USART_GetITStatus(USART3, USART_IT_RXNE) != RESET)
	  {
	    USART_ClearITPendingBit(USART3, USART_IT_RXNE);	//
			if(UART_Com3_Para.Recv_Index < UART_Com3_Para.Recv_Len )
			{
				UART_Com3_Para.buff [UART_Com3_Para.Recv_Index] = USART1->DR;
				UART_Com3_Para.Recv_Index ++;
			}
			if(UART_Com3_Para.Recv_Index == UART_Com3_Para.Recv_Len )
				UART_Com3_Para.RecvStatus  = True;
		}	
}
